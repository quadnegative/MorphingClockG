...
Digitsec::Digitsec(PxMATRIX* d, byte value, uint16_t xo, uint16_t yo, uint16_t color) {
  _display = d;
  _value = value;
  xOffset = xo;
  yOffset = yo;
...
void Digitsec::drawPixel(uint16_t x, uint16_t y, uint16_t c)
{
  _display->drawPixel(xOffset + x, height - (y + yOffset), c);
}

void Digitsec::drawLine(uint16_t x, uint16_t y, uint16_t x2, uint16_t y2, uint16_t c)
{
  _display->drawLine(xOffset + x, height - (y + yOffset), xOffset + x2, height - (y2 + yOffset), c);
}

void Digitsec::drawFillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t c)
{
  _display->fillRect(xOffset + x, height - (y + yOffset), w,h, c);
}
void Digitsec::SetColor(uint16_t c)
{
  _color = c;
}
void Digitsec::DrawColon(uint16_t c)
{
...
void Digitsec::drawSeg(byte seg)
{
  switch (seg) {
...
void Digitsec::Draw(byte value) {
  byte pattern = digitsecBits[value];
...
void Digitsec::Morph2() {
  ...
  for (int i = 0; i <= segWidth; i++)
  {
    ...
    drawLine(segWidth + 1 - i, 1, segWidth + 1 - i, segHeight, black1);
    drawLine(segWidth - i, 1, segWidth - i, segHeight, _color);
    delay(animSpeed);
  }
}

void Digitsec::Morph3() {
  // THREE
  for (int i = 0; i <= segWidth; i++)
  {
    drawLine(0 + i, 1, 0 + i, segHeight, black1);
    drawLine(1 + i, 1, 1 + i, segHeight, _color);
    delay(animSpeed);
  }
}

void Digitsec::Morph4() {
  // FOUR
  for (int i = 0; i < segWidth; i++)
  {
    drawPixel(segWidth - i, segHeight * 2 + 2, black1); // Erase A
    drawPixel(0, segHeight * 2 + 1 - i, _color); // Draw as F
    drawPixel(1 + i, 0, black1); // Erase D
    delay(animSpeed);
  }
}

void Digitsec::Morph5() {
  // FIVE
  for (int i = 0; i < segWidth; i++)
  {
    drawPixel(segWidth + 1, segHeight + 2 + i, black1); // Erase B
...
void Digitsec::Morph6() {
  // SIX
  for (int i = 0; i <= segWidth; i++)
  {
    // Move C right to left
    drawLine(segWidth - i, 1, segWidth - i, segHeight, _color);
    if (i > 0) drawLine(segWidth - i + 1, 1, segWidth - i + 1, segHeight, black1);
    delay(animSpeed);
  }
}

void Digitsec::Morph7() {
  // SEVEN
  for (int i = 0; i <= (segWidth + 1); i++)
  {
    // Move E left to right
    drawLine(0 + i - 1, 1, 0 + i - 1, segHeight, black1);
    drawLine(0 + i, 1, 0 + i, segHeight, _color);

    // Move F left to right
    drawLine(0 + i - 1, segHeight * 2 + 1, 0 + i - 1, segHeight + 2, black1);
    drawLine(0 + i, segHeight * 2 + 1, 0 + i, segHeight + 2, _color);

    // Erase D and G gradually
    drawPixel(1 + i, 0, black1); // D
    drawPixel(1 + i, segHeight + 1, black1); // G
    delay(animSpeed);
  }
}

void Digitsec::Morph8() {
  // EIGHT
  for (int i = 0; i <= segWidth; i++)
  {
    // Move B right to left
    drawLine(segWidth - i, segHeight * 2 + 1, segWidth - i, segHeight + 2, _color);
    if (i > 0) drawLine(segWidth - i + 1, segHeight * 2 + 1, segWidth - i + 1, segHeight + 2, black1);
...
void Digitsec::Morph0() {
  ...
  for (int i = 0; i <= segWidth; i++)
  {
    ...
    if (_value==2) { // If 2 to 0, slide B to F and Flow G to C
      ...
      drawPixel(1+i, segHeight + 1, black1); // Erase G left to right
    ...
    if (_value==3) { // B to F, C to E
      ...
      drawPixel(segWidth - i, segHeight + 1, black1); // G
    }
    
    if (_value==5) { // If 5 to 0, we also need to slide F to B
      if (i<segWidth) {
        if (i>0) drawLine(1 + i, segHeight * 2 + 1, 1 + i, segHeight + 2, black1);
        drawLine(2 + i, segHeight * 2 + 1, 2 + i, segHeight + 2, _color);
      }
    }
    
    if (_value==5 || _value==9) { // If 9 or 5 to 0, Flow G into E
      if (i<segWidth) drawPixel(segWidth - i, segHeight + 1, black1);
...
void Digitsec::Morph1() {
  ...
  for (int i = 0; i <= (segWidth + 1); i++)
  {
    ...
    drawPixel(1 + i, segHeight * 2 + 2, black1); // A
    drawPixel(1 + i, 0, black1); // D
    drawPixel(1 + i, segHeight + 1, black1); // G
...
void Digitsec::Morph(byte newValue) {
  switch (newValue) {
    case 2: Morph2(); break;
    case 3: Morph3(); break;
    case 4: Morph4(); break;
